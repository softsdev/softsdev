...
class class_user_profile_addons{
	...
	public function qa_addons_html(){
		...
		foreach($addons_data as $key=>$values){
			...
			$html.= '<div class="meta version"><b>'.__('Version:', USER_PROFILE_TEXTDOMAIN).'</b> '.$values['version'].'</div>';
			
			if($values['price']==0){
				
				$price = __('Free', USER_PROFILE_TEXTDOMAIN);
				}
			else{
				$price = '$'.$values['price'];
				
				}		
			$html.= '<div class="meta price"><b>'.__('Price:', USER_PROFILE_TEXTDOMAIN).'</b> '.$price.'</div>';							
			$html.= '<div class="meta download"><a href="'.$values['item_link'].'">'.__('Download', USER_PROFILE_TEXTDOMAIN).'</a></div>';				
...
<div class="wrap">
	...
	<div id="icon-tools" class="icon32"><br></div><?php echo "<h2>".sprintf(__('%s - Addons', USER_PROFILE_TEXTDOMAIN), USER_PROFILE_PLUGIN_NAME)."</h2>";?>
