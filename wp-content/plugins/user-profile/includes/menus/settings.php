	...
	public function qa_settings_options($options = array()){
		...
		$section_options = array(
			...
			'user_profile_page_id'=>array(
				...
				'title'=>__('User profile page', USER_PROFILE_TEXTDOMAIN),
				'option_details'=>__('Select user profile page where the shortcode placed <pre>[user_profile]</pre>', USER_PROFILE_TEXTDOMAIN),						
				'input_type'=>'select', 
				'input_args'=> $class_user_profile_functions->get_pages_list(),
			),
			'user_profile_edit_page_id'=>array(
				'css_class'=>'user_profile_edit_page_id',					
				'title'=>__('User profile edit page', USER_PROFILE_TEXTDOMAIN),
				'option_details'=>__('Select user profile edit page where the shortcode placed <pre>[user_profile_edit]</pre>', USER_PROFILE_TEXTDOMAIN),						
				'input_type'=>'select', 
				'input_args'=> $class_user_profile_functions->get_pages_list(),
			),
		...
		$options['<i class="fa fa-folder"></i> '.__('Pages', USER_PROFILE_TEXTDOMAIN)] = apply_filters( 'qa_settings_section_question_post', $section_options );
				...
				elseif( $option_info['input_type'] == 'file' ){
					...
					jQuery("#upload_button_'.$option_key.'").click(function(e) {
						...
						custom_uploader = wp.media.frames.file_frame = wp.media({
							title: "Choose File",
							button: { text: "'.__('Choose File', USER_PROFILE_TEXTDOMAIN).'" },
							multiple: false
						});
...
if(empty($_POST['qa_hidden']))
	...
	{	
		if($_POST['qa_hidden'] == 'Y') {
			...
			?>
			<div class="updated"><p><strong><?php _e('Changes Saved.', USER_PROFILE_TEXTDOMAIN ); ?></strong></p></div>
...
<div class="wrap">
	...
	<div id="icon-tools" class="icon32"><br></div><?php echo "<h2>".sprintf(__('%s Settings', USER_PROFILE_TEXTDOMAIN), USER_PROFILE_PLUGIN_NAME)."</h2>";?>
...
<p class="submit">
                    <input class="button button-primary" type="submit" name="submit" value="<?php _e('Save Changes',USER_PROFILE_TEXTDOMAIN ); ?>" />
                </p>
		</form>
