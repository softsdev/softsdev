...
if(isset($_POST['user_profile_education_hidden'])){
	...
	if(wp_verify_nonce( $nonce, 'nonce_user_profile_education' )){
		...
		$status_html.= '<span class="success"><i class="fa fa-check"></i> '.__('Saved successful.', USER_PROFILE_TEXTDOMAIN).'</span>';
		}
...
<div class="section education">
   <h2><?php echo __('Education', USER_PROFILE_TEXTDOMAIN); ?></h2>
	...
	<?php echo $status_html; ?>
   		...
   		<div class="add-education button"><?php echo __('Add', USER_PROFILE_TEXTDOMAIN); ?></div>
		...
		foreach($user_profile_education as $index=>$education){
			...
			?>
            <div class="item">
                <span class="remove user-profile-tooltip" title="<?php echo __('Delete', USER_PROFILE_TEXTDOMAIN); ?>"><i class="fa fa-times"></i></span>
                <span class="move user-profile-tooltip" title="<?php echo __('Sort', USER_PROFILE_TEXTDOMAIN); ?>"><i class="fa fa-bars" aria-hidden="true"></i></span>        
                <input placeholder="<?php echo __('School/College/University', USER_PROFILE_TEXTDOMAIN); ?>" type="text" name="user_profile_education[<?php echo $index; ?>][school]" value="<?php echo $school; ?>" />
                <input placeholder="<?php echo __('Degree', USER_PROFILE_TEXTDOMAIN); ?>" type="text" name="user_profile_education[<?php echo $index; ?>][degree]" value="<?php echo $degree; ?>" />
                <input size="8" class="user_profile_date" placeholder="<?php echo __('Start date', USER_PROFILE_TEXTDOMAIN); ?>" type="text" name="user_profile_education[<?php echo $index; ?>][start_date]" value="<?php echo $start_date; ?>" />
                <input size="8" class="user_profile_date" placeholder="<?php echo __('End date', USER_PROFILE_TEXTDOMAIN); ?>" type="text" name="user_profile_education[<?php echo $index; ?>][end_date]" value="<?php echo $end_date; ?>" />
                <label><input title="" name="user_profile_education[<?php echo $index; ?>][running]" <?php if(!empty($running)) echo 'checked'; ?>  type="checkbox" value="yes" /> <?php echo __('Running', USER_PROFILE_TEXTDOMAIN); ?></label>
            </div>
        ...
        <input type="submit" value="<?php echo __('Update', USER_PROFILE_TEXTDOMAIN); ?>" />
