...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    <?php if (!is_single()) { ?>
        ...
        <?php $archive_layout = bizprime_get_option('archive_layout'); ?>
        <?php $archive_layout_image = bizprime_get_option('archive_layout_image'); ?>
        ...
        <div class="entry-content twp-entry-content <?php echo esc_attr($full_width_content); ?>">
            ...
            <?php if ('full' == $archive_layout) : ?>
                ...
                /* translators: %s: Name of current post. */
                    wp_kses(__('Continue reading %s <span class="meta-nav">&rarr;</span>', 'bizprime'), array('span' => array('class' => array()))),
                    the_title('<span class="screen-reader-text">"', '"</span>', false)
                ));
                wp_link_pages(array(
                    'before' => '<div class="page-links">' . esc_html__('Pages:', 'bizprime'),
                    'after' => '</div>',
                ));
                ?>
            <?php else : ?>
    ...
    <?php } else { ?>
        <div class="entry-content">
            <?php
            $image_values = get_post_meta($post->ID, 'bizprime-meta-image-layout', true);
            if (empty($image_values)) {
                $values = esc_attr(bizprime_get_option('single_post_image_layout'));
            } else {
