		...
		if ( class_exists( 'WooCommerce' ) && in_array( 'woocommerce' , get_body_class() ) ) {
			...
			if ( is_home() && 'page' == get_option( 'show_on_front' ) ) {
				...
				echo '<h1 class="main-title fadeInDown"><i class="' . esc_html( $blog_title_icon ) . '"></i>' . get_the_title( $blog_page_id ) . '</h1>';
					if ( get_the_excerpt( $blog_page_id ) ) {
			...
			if (!$featured_page_link1) {
				...
				foreach( $recent_posts as $recent ){
					...
					<div class="featured-post featured-post<?php echo $featured_post_number; ?>">
						...
						<?php
						$featured_page_excerpt = get_the_excerpt( $recent["ID"] );
						if ( $featured_page_excerpt == '' ) {
							$featured_page_excerpt = wp_trim_words( strip_shortcodes( get_post_field( 'post_content', $recent["ID"] ) ), 15 );
						}
						if ( '' != $featured_page_excerpt) {
							echo '<p>' . $featured_page_excerpt . '</p>';
						}
				...
				for( $i = 1; $i < 4; $i++ ){
					...
					<div class="featured-post featured-post<?php echo $i ;?>">
						...
						<div class="featured-excerpt">
						<?php
						$featured_page_excerpt = get_the_excerpt( $featured_page_link );
						if ( $featured_page_excerpt == '' ) {
							$featured_page_excerpt = wp_trim_words( strip_shortcodes( get_post_field( 'post_content', $featured_page_link ) ), 15 );
						}
						if ( '' != $featured_page_excerpt ) {
							echo '<p>' . $featured_page_excerpt . '</p>';
						}
